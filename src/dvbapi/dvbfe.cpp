			  ...
			  int timeout)
{
	...
	if (returnval & DVBFE_INFO_FEPARAMS) {
        ...
        result->feparams.inversion = (dvbfe_spectral_inversion)lookupval(kevent.parameters.inversion, 1, dvbfe_spectral_inversion_to_kapi);
		switch(fehandle->type) {
		case FE_QPSK:
			result->feparams.u.dvbs.symbol_rate = kevent.parameters.u.qpsk.symbol_rate;
            result->feparams.u.dvbs.fec_inner = (dvbfe_code_rate)lookupval(kevent.parameters.u.qpsk.fec_inner, 1, dvbfe_code_rate_to_kapi);
			break;

		case FE_QAM:
			result->feparams.u.dvbc.symbol_rate = kevent.parameters.u.qam.symbol_rate;
            result->feparams.u.dvbc.fec_inner = (dvbfe_code_rate)lookupval(kevent.parameters.u.qam.fec_inner, 1, dvbfe_code_rate_to_kapi);
            result->feparams.u.dvbc.modulation = (dvbfe_dvbc_mod)lookupval(kevent.parameters.u.qam.modulation, 1, dvbfe_dvbc_mod_to_kapi);
			break;

		case FE_OFDM:
            result->feparams.u.dvbt.bandwidth = (dvbfe_dvbt_bandwidth)lookupval(kevent.parameters.u.ofdm.bandwidth, 1, dvbfe_dvbt_bandwidth_to_kapi);
            result->feparams.u.dvbt.code_rate_HP = (dvbfe_code_rate)lookupval(kevent.parameters.u.ofdm.code_rate_HP, 1, dvbfe_code_rate_to_kapi);
            result->feparams.u.dvbt.code_rate_LP = (dvbfe_code_rate)lookupval(kevent.parameters.u.ofdm.code_rate_LP, 1, dvbfe_code_rate_to_kapi);
            result->feparams.u.dvbt.constellation = (dvbfe_dvbt_const)lookupval(kevent.parameters.u.ofdm.constellation, 1, dvbfe_dvbt_const_to_kapi);
            result->feparams.u.dvbt.transmission_mode = (dvbfe_dvbt_transmit_mode)lookupval(kevent.parameters.u.ofdm.transmission_mode, 1, dvbfe_dvbt_transmit_mode_to_kapi);
            result->feparams.u.dvbt.guard_interval = (dvbfe_dvbt_guard_interval)lookupval(kevent.parameters.u.ofdm.guard_interval, 1, dvbfe_dvbt_guard_interval_to_kapi);
            result->feparams.u.dvbt.hierarchy_information = (dvbfe_dvbt_hierarchy)lookupval(kevent.parameters.u.ofdm.hierarchy_information, 1, dvbfe_dvbt_hierarchy_to_kapi);
			break;

		case FE_ATSC:
            result->feparams.u.atsc.modulation = (dvbfe_atsc_mod)lookupval(kevent.parameters.u.vsb.modulation, 1, dvbfe_atsc_mod_to_kapi);
			break;
		}
	}
	      ...
	      int timeout)
{
    ...
    kparams.inversion = (fe_spectral_inversion_t)lookupval(params->inversion, 0, dvbfe_spectral_inversion_to_kapi);
	switch(fehandle->type) {
	case FE_QPSK:
        kparams.u.qpsk.symbol_rate = params->u.dvbs.symbol_rate;
        kparams.u.qpsk.fec_inner = (fe_code_rate_t)lookupval(params->u.dvbs.fec_inner, 0, dvbfe_code_rate_to_kapi);
		break;

	case FE_QAM:
		kparams.u.qam.symbol_rate = params->u.dvbc.symbol_rate;
        kparams.u.qam.fec_inner = (fe_code_rate_t)lookupval(params->u.dvbc.fec_inner, 0, dvbfe_code_rate_to_kapi);
        kparams.u.qam.modulation = (fe_modulation_t)lookupval(params->u.dvbc.modulation, 0, dvbfe_dvbc_mod_to_kapi);
		break;

	case FE_OFDM:
        kparams.u.ofdm.bandwidth = (fe_bandwidth_t)lookupval(params->u.dvbt.bandwidth, 0, dvbfe_dvbt_bandwidth_to_kapi);
        kparams.u.ofdm.code_rate_HP = (fe_code_rate_t)lookupval(params->u.dvbt.code_rate_HP, 0, dvbfe_code_rate_to_kapi);
        kparams.u.ofdm.code_rate_LP = (fe_code_rate_t)lookupval(params->u.dvbt.code_rate_LP, 0, dvbfe_code_rate_to_kapi);
        kparams.u.ofdm.constellation = (fe_modulation_t)lookupval(params->u.dvbt.constellation, 0, dvbfe_dvbt_const_to_kapi);
        kparams.u.ofdm.transmission_mode = (fe_transmit_mode_t)
			lookupval(params->u.dvbt.transmission_mode, 0, dvbfe_dvbt_transmit_mode_to_kapi);
        kparams.u.ofdm.guard_interval = (fe_guard_interval_t)
			lookupval(params->u.dvbt.guard_interval, 0, dvbfe_dvbt_guard_interval_to_kapi);
        kparams.u.ofdm.hierarchy_information = (fe_hierarchy_t)
			lookupval(params->u.dvbt.hierarchy_information, 0, dvbfe_dvbt_hierarchy_to_kapi);
                break;

	case FE_ATSC:
        kparams.u.vsb.modulation = (fe_modulation_t)lookupval(params->u.atsc.modulation, 0, dvbfe_atsc_mod_to_kapi);
		break;
